...
M.setup_term_number_toggle = function()
    vim.api.nvim_exec([[
      augroup TermNumberToggle
        autocmd!
        autocmd TermOpen * lua require("my_module").disable_line_numbers()
        autocmd TermClose * lua require("my_module").enable_line_numbers()
      augroup END
    ]], false)
end
...
M.setup_term_statusline_highlight = function()
    ...
    autocmd!
    autocmd TermOpen * lua require("my_module").set_term_statusline_highlight()
    augroup END
    ]], false)
end
...
M.setup_terminal_config = function()
    M.setup_term_number_toggle()
    M.setup_term_highlight()
    M.setup_term_statusline()
    M.setup_term_statusline_highlight()
end

M.setup_terminal_autocommands = function()
    M.setup_term_close_autocmd()
    M.setup_term_enter_autocmd()
    M.setup_term_leave_autocmd()
end

M.setup_terminal_autocommands()
M.setup_terminal_config()
...
vim.api.nvim_set_keymap('n', '<leader>q', ':lua cd_and_open_term_mod()<CR>', { noremap = true, silent = true })
